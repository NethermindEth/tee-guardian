...
# EDK2 OVMF Firmware for AMD SEV-SNP
#
# Builds OVMF with AMD SEV-SNP guest support (AmdSev platform).
# This firmware is required for launching SEV-SNP confidential VMs.
#
# IMPORTANT: SEV-SNP with Linux 6.11+ requires loading via -bios, not pflash.
# This is due to guest_memfd incompatibility with memory-mapped flash devices.
#
# Output: OVMF.fd - Combined code and variable store firmware image
#
# Usage in QEMU:
#   -bios /path/to/OVMF.fd
#   -object sev-snp-guest,id=sev0,policy=0x30000,cbitpos=51,...
...
, secureBoot ? false
, buildShell ? true
, smmSupport ? false   # SMM is typically disabled for SEV-SNP
}:
...
stdenv.mkDerivation {
  ...
  buildPhase = ''
    ...
    # Build SEV firmware
    # Note: OvmfPkg/AmdSev/AmdSevX64.dsc is the SEV-SNP specific platform
    echo "Building EDK2 SEV-SNP firmware (${buildType})..."
    build -a X64 -b ${buildType} -t GCC5 \
      ...
      -D SMM_REQUIRE=${if smmSupport then "TRUE" else "FALSE"}
  ...
  installPhase = ''
    ...
    # Copy firmware image
    cp Build/AmdSev/${buildType}_GCC5/FV/OVMF.fd $out/OVMF.fd
    ...
    cat > $out/metadata.json <<EOF
    {
      ...
      "smmSupport": ${if smmSupport then "true" else "false"},
      "version": "${version}",
      "notes": "Load via -bios for SEV-SNP with guest_memfd (Linux 6.11+)"
    }
    EOF

    echo "Built SEV-SNP firmware: $(ls -lh $out/OVMF.fd | awk '{print $5}')"
  ...
  passthru = {
    inherit buildType secureBoot smmSupport;
  };

  meta = with lib; {
    description = "EDK2 OVMF firmware for AMD SEV-SNP confidential VMs";
    longDescription = ''
      OVMF firmware built with AMD SEV-SNP support. This firmware must be
      loaded via QEMU's -bios parameter (not pflash) when using Linux 6.11+
      with guest_memfd, as the new private memory backend is incompatible
      with memory-mapped flash devices.

      For QEMU usage:
        -bios ${placeholder "out"}/OVMF.fd
